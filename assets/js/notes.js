...
// ==========================================
// FUNGSI SEMENTARA UNTUK MIGRASI NOTES LOKAL
// ==========================================
window.migrasiNotesLokalKeFirestore = async function() {
    // PENTING: Ganti dengan nama key localStorage yang kamu pakai untuk menyimpan notes sebelumnya
    // (Bisa dicek di F12 -> Application -> Local Storage)
    const keyLokal = 'taufik_notes_db_v1'; 
    const rawData = localStorage.getItem(keyLokal);

    if (!rawData) {
        console.warn(`Data lokal dengan key '${keyLokal}' tidak ditemukan di browser ini.`);
        showToast("Data lokal tidak ditemukan.");
        return;
    }

    try {
        const dataLama = JSON.parse(rawData);
        
        // Memastikan formatnya berupa array
        if (!Array.isArray(dataLama) || dataLama.length === 0) {
            console.warn("Format data catatan lokal kosong atau tidak sesuai.");
            return;
        }

        showToast("Sedang memindahkan catatan ke Cloud...");
        let jumlahBerhasil = 0;

        // Kita pakai fitur "Batch" dari Firestore agar bisa mengupload banyak data sekaligus dengan cepat
        const batch = db.batch();

        dataLama.forEach(note => {
            // Pastikan format id ada, kalau tidak kita buatkan ID baru
            const idNote = note.id || 'NOTE-' + Date.now() + Math.floor(Math.random() * 1000);
            
            // Konversi struktur data lama ke format standar yang baru
            const noteBaru = {
                id: idNote,
                title: note.title || 'Tanpa Judul',
                content: note.content || '',
                category: note.category || 'misc',
                tags: note.tags || [],
                color: note.color || '#ffffff',
                isPinned: note.isPinned || false,
                isArchived: note.isArchived || false,
                isShared: note.isShared || false,
                shareMode: note.shareMode || 'view',
                order: note.order || 0,
                created_at: note.created_at || new Date().toISOString()
            };

            const docRef = db.collection("notes").doc(idNote);
            batch.set(docRef, noteBaru);
            jumlahBerhasil++;
        });
        ...
        console.log(`Sukses! ${jumlahBerhasil} catatan berhasil dipindah ke Firestore.`);
        showToast(`${jumlahBerhasil} catatan berhasil dimigrasi!`, "success");

    } catch (e) {
        console.error("Gagal saat melakukan migrasi:", e);
